...
export default function PriceCard({
  data: {
    ...
    description,
    description1,
    buttonText = "Visit The CorpoRealizers",
    points,
  },
}) {
  const handleClick = () => {
    ("hi");
    window.location.href = "https://www.thecorporealizers.in";
  };
  const handleClickPortfolio = () => {
    ("hi");
    window.location.href = "https://www.itstanuj.in";
  };
  return (
    <Card
      ...
      <Box>
        <Box className="package__header" sx={styles.pricingHeader}>
          <Button onClick={() => handleClickPortfolio()}>
            <Text styles={{ color: "white" }}>- {description}</Text>
          </Button>
